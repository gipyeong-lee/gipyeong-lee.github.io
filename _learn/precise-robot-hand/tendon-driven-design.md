...
- '예제 2: 전원 분기 분배 및 보호
  ...
  각각 9.2 A, 9.2 A, 6.9 A이다. 4대 액추에이터 분기의 이론 피크 9.2 A는 10 A 퓨즈와 11.5 A 어댑터 정격보다 낮지만,
  이 수치만으로 안전성이나 동작 순서를 보장하지 않는다. 퓨즈 제조사 시간-전류 곡선과 어댑터 OCP 특성을 함께 검토해 보호 협조를 확인한다 [S24,
  S25].'
lab:
...
- question: 3개의 12 V 전원 어댑터(각 11.5 A)를 사용하는 이유로 적절한 것은?
  ...
  - 전원 노이즈를 제거하기 위해
  answer_index: 2
  explanation: 총 11대의 액추에이터 피크 전류를 안전하게 분산하고, 각 분기를 10 A 퓨즈로 보호하여 시스템 과전류 위험을 낮추기
    위함이다 [S11, S15, S25].
completion_criteria:
...
### 2. 기계적 이득과 구동기 선정
XM430-W350-T 스마트 액추에이터는 스톨 토크 4.1 N·m를 제공한다 [S11]. 텐던은 회전축에서 캡스턴 반경을 통해 힘을 변환하므로, 액추에이터의 토크 출력은 텐던의 장력으로 치환된다. 전체 시스템은 11대의 액추에이터를 사용하며, 피크 전류 합계는 약 25.3 A에 도달할 수 있다 [S11]. 따라서 이를 안정적으로 공급하기 위해 총 3개의 독립된 12 V 전원 분기를 구성하며, 각 분기는 독립적인 10 A 퓨즈 보호를 통해 과전류를 방지한다 [S15, S24, S25].
